...
PDMaas是一款优秀的国产数据建模工具，所有操作系统都能用，主打轻便好看效率高。它经由PDMan→PDManer→PDMaas的产品迭代。它能帮你：
* 拖拽画数据表和ER关系图，概念/逻辑/物理三层建模
* 把流程图、思维导图整合到一起看
* 有标准字段库和枚举代码库
* 自动生成Word、HTML、Markdown文档和Java/Python等代码
* 模型和数据库双向同步，自动生成DDL差异脚本
* 支持多版本管理，便捷追溯模型历史变更
* 支持多种数据库，包括主流数据库（如MySQL、PostgreSQL）、国产数据库、大数据MPP库全兼容
...
> PDMass支持Windows、MacOS、Linux等操作系统，并深度兼容国产操作系统（如麒麟、统信）

其软件界面如下：

![PDMaas](assets/pdmass.png "PDMaas")

> 官方网站：https://www.yonsum.com/

### 2、dev-sidecar

**重点推荐**这个【dev-sidecar】开发者边车，这个工具主要用于GitHub、NPM加速等。我们在开发过程中经常需要访问GitHub一些开源软件或开源代码库、资料等，在访问GitHub时如果不借助代理等其他手段的情况下，网络时好时坏，相当不稳定。而通过dev-sidecar可以稳定的访问GitHub等，实在是开发者的福音。
...
这里**安利**一款开发利器：FlyEnv。FlyEnv 是一款专为现代开发者设计的一体化环境管理工具。其通过多版本切换、全栈技术支持、无缝跨平台体验，简化安装配置等开发流程。其前身虽是PHPStudy，目前FleyEnv已支持Java、PHP、Python、Go、NodeJS等语言，MySQL、MariaDB、PostgreSQL、MongoDB等数据库，Redis、RabbitMQ、ElasticSearch等中间件，Apache、Nginx、Tomcat、Consul、Minio等服务。
...
> 在线地址：https://uutool.cn/type/code/
